...
exports.createMusicTask = async (req, res) => {
  try {
    ...
    let result = await MusicTask.createTask(req.body);
    if (result.success) {
      logger.info(
        {
          clientIP: req.socket.remoteAddress,
...
exports.updateMusicTask = async (req, res) => {
  try {
    ...
    let result = await MusicTask.updateTask(taskId, req.body);
    if (result.success) {
      logger.info(
        {
          clientIP: req.socket.remoteAddress,
...
exports.getAllTrainingSessions = async (req, res) => {
  try {
    ...
    logger.info(
      ...
      `Request received to fetch all training sessions for balagruhaId: ${balagruhaId}`
    );
    let result = await TrainingSession.getAllTrainingSessions({ balagruhaId });
    if (result.success) {
      logger.info(
        {
          clientIP: req.socket.remoteAddress,
...
exports.getSportsInsights = async (req, res) => {
  try {
    ...
    logger.info(
      ...
      `Request received to fetch sports overview for balagruhaId: ${balagruhaId}`
    );
    let result = await MusicTask.getMusicInsights({ balagruhaId, date });
    if (result.success) {
      logger.info(
        {
          clientIP: req.socket.remoteAddress,
...
exports.addOrUpdateSportsTaskAttachment = async (req, res) => {
  try {
    ...
    let isOfflineReq = isRequestFromLocalhost(req);
    let result = await MusicTask.addOrUpdateMusicTaskAttachment({
      taskId,
      attachments,
...
exports.addCommentToSportsTask = async (req, res) => {
  try {
    ...
    let isOfflineReq = isRequestFromLocalhost(req);
    let result = await MusicTask.addCommentsToMusicTask({
      taskId,
      comments: comments,
      createdById: req.user._id,
      attachments,
