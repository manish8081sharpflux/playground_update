...
/**
 * Fix WTF transaction types and add wtfPinId
 */
async function fixWtfTransactions() {
  try {
    console.log("🚀 Starting WTF transaction fix...");
    ...
    for (const coinRecord of coinRecords) {
      ...
      for (const transaction of coinRecord.transactions) {
        ...
        // Step 1: Fix transaction type
        if (
          transaction.type === "earned" &&
          transaction.description.includes("WTF pin")
        ) {
        ...
        // Step 2: Add wtfPinId if missing
        if (transaction.type === "wtf_pin_creation" && !transaction.wtfPinId) {
    ...
    errorLogger.error({ error: error.message }, "WTF transaction fix failed");
    throw error;
  }
}
...
async function verifyFixes() {
  try {
    ...
    const correctTypeTransactions = await Coin.countDocuments({
      "transactions.type": "wtf_pin_creation",
    });
    console.log(
      `📊 Transactions with correct type (wtf_pin_creation): ${correctTypeTransactions}`
    );
    ...
    console.log(
      `📊 wtf_pin_creation transactions without wtfPinId: ${transactionsWithoutPinId}`
    );

    // Sample a fixed transaction
    ...
    if (sampleCoin) {
      const fixedTransaction = sampleCoin.transactions.find(
        (t) => t.type === "wtf_pin_creation" && t.wtfPinId
      );
      if (fixedTransaction) {
        console.log("\n✅ Sample fixed transaction:");
        console.log(`   Transaction ID: ${fixedTransaction._id}`);
        console.log(`   Type: ${fixedTransaction.type}`);
        console.log(`   Description: ${fixedTransaction.description}`);
...
async function runMigration() {
  try {
    ...
    console.log("🔄 Running WTF transaction fix...\n");

    const result = await fixWtfTransactions();
