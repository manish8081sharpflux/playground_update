    ...
    try {
      ...
      if (balagruhaId) {
        const studentList = users.filter(
          (u) => u.role === "student" && u.balagruhaId === balagruhaId,
        );
            ...
            <table className="detail-table">
              <tbody>
                ...
                <tr>
                  <td>Change:</td>
                  <td
                    className={`${performanceData.change.includes("+") ? "positive" : "negative"}`}
                  >
...
const SportCoachDashboard = () => {
  ...
  const getBalagruhaList = async () => {
    try {
      ...
      const actualBalagruhas = (response?.data || []).filter(
        (b) => b._id !== "STOCK",
      );
  ...
  const TrainingFilter = ({ onFilterChange, filters, balagruhas }) => {
    ...
    useEffect(() => {
      if (
        balagruhas?.length > 0 &&
        (!filters.balagruhaId || filters.balagruhaId.length === 0)
      ) {
    ...
    const handleSelectAllVisible = () => {
      ...
      if (allCurrentlySelected) {
        ...
        const newSelected = [
          ...new Set([...(filters.balagruhaId || []), ...visibleBalagruhaIds]),
        ];
        onFilterChange({
          ...filters,
          balagruhaId: newSelected,
      ...
      <div className="training-filter" ref={filterRef}>
        <div className="filter-buttons">
          <div style={{ position: "relative" }}>
            ...
            >
              Balagruha{" "}
              {filters.balagruhaId?.length > 0 &&
                `(${filters.balagruhaId.length})`}
            </button>
  ...
  const sportCoachMenu = [
    ...
    // { id: 8, name: "Reports", activeTab: "reports" },
  ];
                    ...
                    <div className="dashboard-card tasks-overview">
                      ...
                      <div className="recent-tasks">
                        ...
                        {tasks.slice(0, 3).map((task) => (
                          ...
                          >
                            <div
                              className={`task-priority ${task.priority?.toLowerCase() || "medium"}`}
                            ></div>
                            ...
                            <div
                              className={`task-status ${task.status?.replace(/\s+/g, "-").toLowerCase() || "pending"}`}
                            >
                              {task.status || "Pending"}
                            </div>
                          </div>
            ...
            <div className="sport-students-section">
              ...
              <div className="sport-filter-container">
                ...
                <div className="sport-filter-actions">
                  ...
                  <button className="sport-filter-button clear">
                    Clear Filters
                  </button>
                  <div className="sport-view-options">
                    <button
                      className="sport-view-button active"
                      title="Table View"
                    >
                      <span>📋</span>
                    </button>
                  ...
                  <tbody>
                    {assignedStudents.map((student) => (
                      <tr key={student.id}>
                        ...
                        <td>
                          {student.sport === "Basketball"
                            ? "Practice 50 free throws with proper form"
                            : student.sport === "Swimming"
                              ? "Focus on arm rotation and breathing technique"
                              : student.sport === "Soccer"
                                ? "Practice penalty kicks from different angles"
                                : "Practice serves targeting different court zones"}
                        </td>
                        <td>
                          {student.sport === "Basketball"
                            ? "45 mins"
                        ...
                        <td>
                          {student.name === "Alex Johnson"
                            ? "Improving steadily, focus on follow-through"
                            : student.name === "Maya Patel"
                              ? "Excellent progress, work on turns"
                              : student.name === "Tyler Smith"
                                ? "Good effort, needs more consistency"
                      ...
                      <tbody>
                        {filteredPerformanceData.map((data, index) => (
                          <tr key={index} className="performance-table-row">
                            ...
                            <td
                              className={`change-value ${data.change.includes("+") ? "positive" : "negative"}`}
                            >
                              {data.change}
                            </td>
                      ...
                      {filteredPerformanceData.map((data, index) => (
                        <li key={index} className="performance-list-item">
                          <div className="performance-list-header">
                            ...
                            <div
                              className={`change-badge ${data.change.includes("+") ? "positive" : "negative"}`}
                            >
                              {data.change}
                            </div>
                          </div>
